...
4. object_terms 必须来自章节标题或正文中的对象词。
5. 不要判断是否符合规范，不要生成整改意见，不要引用规范条文，只抽取施工方案中已经写明的证据点。
6. confidence 为 0-1 小数，表示该证据点抽取可信度。
7. 如果章节正文中没有可审查的具体措施、参数、禁止项、条件项或对象关系，则 evidence_points 输出空数组。
8. 每个evidence_text为原文其中一段，且不允许重复，所有的evidence_text连起来等于原文。
9. 如果有表格，则每一行作为一个evidence_points、evidence_text

拆分示例：
原文：
“每根立柱底部应设置垫板，垫板厚度不得小于50mm，支设上层支架时下层支架严禁拆除，且上层支架的立柱位置应与下层支架立柱位置对应。梁下立杆如有不对应情况，下层顶板立杆局部加设立杆，保证上下层立杆对应。”

步骤：1、原文拆分成3个evidence_text，每个evidence_text独立且不会重复。
